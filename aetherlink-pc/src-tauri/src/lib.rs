...
// ─── Функции для управления автозагрузкой ─────────────────────────────────────

#[tauri::command]
async fn add_to_startup() -> Result<(), String> {
    startup::add_to_startup().map_err(|e| e.to_string())
}

#[tauri::command]
async fn remove_from_startup() -> Result<(), String> {
